...
static void dxva2_destroy_decoder(AVCodecContext *s)
{
    ...
    if (ctx->decoder) {
		ctx->decoder->Release();//IDirectXVideoDecoder_Release(ctx->decoder);
        ctx->decoder = NULL;
    }
}
...
static void dxva2_uninit(AVCodecContext *s)
{
	...
	if (ctx->decoder_service)
		ctx->decoder_service->Release();//IDirectXVideoDecoderService_Release(ctx->decoder_service);

	if (ctx->d3d9devmgr && ctx->deviceHandle != INVALID_HANDLE_VALUE)
		ctx->d3d9devmgr->CloseDeviceHandle(ctx->deviceHandle);//IDirect3DDeviceManager9_CloseDeviceHandle(ctx->d3d9devmgr, ctx->deviceHandle);

	if (ctx->d3d9devmgr)
		ctx->d3d9devmgr->Release();//IDirect3DDeviceManager9_Release(ctx->d3d9devmgr);
...
static int dxva2_alloc(AVCodecContext *s)
{
	...
	hr = ctx->d3d9devmgr->ResetDevice(ctx->d3d9device, resetToken);//IDirect3DDeviceManager9_ResetDevice(ctx->d3d9devmgr, ctx->d3d9device, resetToken);
    if (FAILED(hr)) {
        av_log(NULL, loglevel, "Failed to bind Direct3D device to device manager\n");
        goto fail;
    }

	hr = ctx->d3d9devmgr->OpenDeviceHandle(&ctx->deviceHandle);//IDirect3DDeviceManager9_OpenDeviceHandle(ctx->d3d9devmgr, &ctx->deviceHandle);
    if (FAILED(hr)) {
	...
	hr = ctx->d3d9devmgr->GetVideoService(ctx->deviceHandle, IID_IDirectXVideoDecoderService, (void **)&ctx->decoder_service);
		//IDirect3DDeviceManager9_GetVideoService(ctx->d3d9devmgr, ctx->deviceHandle, &IID_IDirectXVideoDecoderService, (void **)&ctx->decoder_service);
    if (FAILED(hr)) {
                                           ...
                                           DXVA2_ConfigPictureDecode *config)
{
	...
	hr = ctx->decoder_service->GetDecoderConfigurations(*device_guid, desc, NULL, &cfg_count, &cfg_list);
		//IDirectXVideoDecoderService_GetDecoderConfigurations(ctx->decoder_service, device_guid, desc, NULL, &cfg_count, &cfg_list);
    if (FAILED(hr)) {
...
static int dxva2_create_decoder(AVCodecContext *s)
{
	...
	hr = ctx->decoder_service->GetDecoderDeviceGuids(&guid_count, &guid_list);
		//IDirectXVideoDecoderService_GetDecoderDeviceGuids(ctx->decoder_service, &guid_count, &guid_list);
    if (FAILED(hr)) {
    ...
    for (i = 0; dxva2_modes[i].guid; i++) {
		...
		hr = ctx->decoder_service->GetDecoderRenderTargets(*(mode->guid), &target_count, &target_list);
			//IDirectXVideoDecoderService_GetDecoderRenderTargets(ctx->decoder_service, mode->guid, &target_count, &target_list);
        if (FAILED(hr)) {
            continue;
        }
	...
	hr = ctx->decoder_service->CreateSurface(FFALIGN(s->coded_width, surface_alignment),
		                                     ...
		                                     ctx->surfaces, NULL);
#if 0
		IDirectXVideoDecoderService_CreateSurface(ctx->decoder_service,
	...
	hr = ctx->decoder_service->CreateVideoDecoder(device_guid, &desc, &config, ctx->surfaces, ctx->num_surfaces, &ctx->decoder);
#if 0
		IDirectXVideoDecoderService_CreateVideoDecoder(ctx->decoder_service, &device_guid,
