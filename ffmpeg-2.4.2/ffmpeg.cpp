...
struct OptionDef const * options = NULL;
struct HWAccel const * hwaccels = NULL;
#endif
#if HAVE_TERMIOS_H
...
static void update_benchmark(const char *fmt, ...)
{
    if (do_benchmark_all) {
        ...
        if (fmt) {
            ...
            va_end(va);
            printf("bench: %8I64u %s \n", t - current_time, buf);//"PRIu64"
        }
        current_time = t;
    }
}
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (!ost->st->codec->extradata_size && ost->enc_ctx->extradata_size) {
        ost->st->codec->extradata =(uint8_t *) av_mallocz(ost->enc_ctx->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
     ...
     if(
      ...
      if (pkt->dts < max) {
        ...
        av_log(s, loglevel, "Non-monotonous DTS in output stream "
               "%d:%d; previous: %I64d, current: %I64d; ",//"PRId64""PRId64"
               ost->file_index, ost->st->index, ost->last_mux_dts, pkt->dts);
        if (exit_on_error) {
            av_log(NULL, AV_LOG_FATAL, "aborting.\n");
            exit_program(1);
        }
        av_log(s, loglevel, "changing to %I64d. This may result "//"PRId64"
        ...
        if (pkt->dts != AV_NOPTS_VALUE &&
            ...
            pkt->dts > pkt->pts) {
            av_log(s, AV_LOG_WARNING, "Invalid DTS: %I64d PTS: %I64d in output stream %d:%d\n",//"PRId64""PRId64"
                   pkt->dts, pkt->pts,
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "muxer <- type:%s "
				...
				av_ts_make_string(av_ts_string, pkt->pts),//av_ts2str(pkt->pts), 
				av_ts_make_time_string(av_ts_string, pkt->pts, &ost->st->time_base),//av_ts2timestr(pkt->pts, &ost->st->time_base),
                av_ts_make_string(av_ts_string, pkt->dts),//av_ts2str(pkt->dts), 
				av_ts_make_time_string(av_ts_string, pkt->dts, &ost->st->time_base), //av_ts2timestr(pkt->dts, &ost->st->time_base),
                pkt->size
              );
    }
                         ...
                         AVFrame *frame)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "encoder <- type:audio "
               ...
               av_ts_make_string(av_ts_string, frame->pts),//av_ts2str(frame->pts), 
			   av_ts_make_time_string(av_ts_string, frame->pts, &enc->time_base),//av_ts2timestr(frame->pts, &enc->time_base),
                            ...
                            AVSubtitle *sub)
{
    ...
    for (i = 0; i < nb; i++) {
        ...
        sub->pts               += av_rescale_q(sub->start_display_time, av_make_q( 1, 1000 ), AV_TIME_BASE_Q);
        ...
        pkt.duration = av_rescale_q(sub->end_display_time, av_make_q( 1, 1000 ), ost->st->time_base);
        if (enc->codec_id == AV_CODEC_ID_DVB_SUBTITLE) {
    ...
    if (s->oformat->flags & AVFMT_RAWPICTURE &&
        ...
        if (debug_ts) {
            av_log(NULL, AV_LOG_INFO, "encoder <- type:video "
                   "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
				   av_ts_make_string(av_ts_string, in_picture->pts),//av_ts2str(in_picture->pts), 
				   av_ts_make_time_string(av_ts_string, in_picture->pts, &enc->time_base),//av_ts2timestr(in_picture->pts, &enc->time_base),
                   enc->time_base.num, enc->time_base.den);
        }
        ...
        if (got_packet) {
            if (debug_ts) {
                av_log(NULL, AV_LOG_INFO, "encoder -> type:video "
                       "pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s\n",
					   av_ts_make_string(av_ts_string, pkt.pts),//av_ts2str(pkt.pts), 
					   av_ts_make_time_string(av_ts_string, pkt.pts, &enc->time_base),//av_ts2timestr(pkt.pts, &enc->time_base),
					   av_ts_make_string(av_ts_string, pkt.dts),//av_ts2str(pkt.dts), 
					   av_ts_make_time_string(av_ts_string, pkt.dts, &enc->time_base)//av_ts2timestr(pkt.dts, &enc->time_base)
					   );
            }
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (debug_ts) {
                    av_log(NULL, AV_LOG_INFO, "filter -> pts:%s pts_time:%s time_base:%d/%d\n",
						    av_ts_make_string(av_ts_string, filtered_frame->pts),//av_ts2str(filtered_frame->pts), 
							av_ts_make_time_string(av_ts_string,  filtered_frame->pts, &enc->time_base),//av_ts2timestr(filtered_frame->pts, &enc->time_base),
                            enc->time_base.num, enc->time_base.den);
                }
...
static void print_final_stats(int64_t total_size)
{
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        for (j = 0; j < f->nb_streams; j++) {
            ...
            av_log(NULL, AV_LOG_VERBOSE, "%I64u packets read (%I64u bytes); ",//"PRIu64""PRIu64"
                   ist->nb_packets, ist->data_size);

            if (ist->decoding_needed) {
                av_log(NULL, AV_LOG_VERBOSE, "%I64u frames decoded",//"PRIu64"
                       ist->frames_decoded);
                if (type == AVMEDIA_TYPE_AUDIO)
                    av_log(NULL, AV_LOG_VERBOSE, " (%I64u samples)", ist->samples_decoded);//"PRIu64"
                av_log(NULL, AV_LOG_VERBOSE, "; ");
            }
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        for (j = 0; j < of->ctx->nb_streams; j++) {
            ...
            if (ost->encoding_needed) {
                av_log(NULL, AV_LOG_VERBOSE, "%I64u frames encoded",//"PRIu64"
                       ost->frames_encoded);
                if (type == AVMEDIA_TYPE_AUDIO)
                    av_log(NULL, AV_LOG_VERBOSE, " (%I64u samples)", ost->samples_encoded);//"PRIu64"
                av_log(NULL, AV_LOG_VERBOSE, "; ");
            }

            av_log(NULL, AV_LOG_VERBOSE, "%I64u packets muxed (%I64u bytes); ",//"PRIu64""PRIu64"
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (qp_hist) {
                ...
                if (qp >= 0 && qp < FF_ARRAY_ELEMS(qp_histogram))
                    qp_histogram[qp]++;
                for (j = 0; j < 32; j++)
                    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%X", (int)lrintf(log2(qp_histogram[j] + 1.0f)));
            }
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
    ...
    if (ost->st->codec->codec_type == AVMEDIA_TYPE_AUDIO && pkt->dts != AV_NOPTS_VALUE) {
        ...
        opkt.dts = opkt.pts = av_rescale_delta(ist->st->time_base, pkt->dts,
                                               av_make_q(1, ist->dec_ctx->sample_rate), duration, &ist->filter_in_rescale_delta_last,
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE)
        decoded_frame->pts = av_rescale_delta(decoded_frame_tb, decoded_frame->pts,
                                              av_make_q(1, avctx->sample_rate), decoded_frame->nb_samples, &ist->filter_in_rescale_delta_last,
                                              av_make_q(1, avctx->sample_rate));
    for (i = 0; i < ist->nb_filters; i++) {
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "decoder -> ist_index:%d type:video "
               ...
               ist->st->index, 
			   av_ts_make_string(av_ts_string, decoded_frame->pts),//av_ts2str(decoded_frame->pts),
			   av_ts_make_time_string(av_ts_string, decoded_frame->pts, &ist->st->time_base),//av_ts2timestr(decoded_frame->pts, &ist->st->time_base),
               best_effort_timestamp,
			   av_ts_make_time_string(av_ts_string, best_effort_timestamp, &ist->st->time_base),//av_ts2timestr(best_effort_timestamp, &ist->st->time_base),
...
static void report_new_stream(int input_index, AVPacket *pkt)
{
    ...
    av_log(file->ctx, AV_LOG_WARNING,
           ...
           pkt->pos, 
		   av_ts_make_time_string(av_ts_string, pkt->dts, &st->time_base)//av_ts2timestr(pkt->dts, &st->time_base)
		   );
                ...
                if (ost->frame_aspect_ratio.num) { // overridden by the -aspect cli option
                    sar =
                        av_mul_q(ost->frame_aspect_ratio,
                                 av_make_q( enc_ctx->height, enc_ctx->width ));
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                enc_ctx->sample_aspect_ratio = ost->st->sample_aspect_ratio =
                    ...
                    av_mul_q(ost->frame_aspect_ratio, av_make_q( enc_ctx->height, enc_ctx->width )) :
                ...
                break;
            case AVMEDIA_TYPE_SUBTITLE:
                enc_ctx->time_base = av_make_q(1, 1000);
                if (!enc_ctx->width) {
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        ost->st->time_base = av_add_q(ost->enc_ctx->time_base, av_make_q(0, 1));
    }
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
			   ...
			   av_ts_make_string(av_ts_string, ist->next_dts),//av_ts2str(ist->next_dts),
			   av_ts_make_time_string(av_ts_string, ist->next_dts, &AV_TIME_BASE_Q),//av_ts2timestr(ist->next_dts, &AV_TIME_BASE_Q),
			   av_ts_make_string(av_ts_string, ist->next_pts),//av_ts2str(ist->next_pts),
			   av_ts_make_time_string(av_ts_string, ist->next_pts, &AV_TIME_BASE_Q),//av_ts2timestr(ist->next_pts, &AV_TIME_BASE_Q),
			   av_ts_make_string(av_ts_string, pkt.pts),//av_ts2str(pkt.pts),
			   av_ts_make_time_string(av_ts_string, pkt.pts, &ist->st->time_base),//av_ts2timestr(pkt.pts, &ist->st->time_base),
			   av_ts_make_string(av_ts_string, pkt.dts),//av_ts2str(pkt.dts),
			   av_ts_make_time_string(av_ts_string, pkt.dts, &ist->st->time_base),//av_ts2timestr(pkt.dts, &ist->st->time_base),
			   av_ts_make_string(av_ts_string, input_files[ist->file_index]->ts_offset),//av_ts2str(input_files[ist->file_index]->ts_offset),
			   av_ts_make_time_string(av_ts_string, input_files[ist->file_index]->ts_offset, &AV_TIME_BASE_Q)//av_ts2timestr(input_files[ist->file_index]->ts_offset, &AV_TIME_BASE_Q)
			   );
    }
