    ...
    if (ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
        ...
        if (ost->enc_ctx->strict_std_compliance <= FF_COMPLIANCE_UNOFFICIAL) {
            if (ost->enc_ctx->codec_id == AV_CODEC_ID_MJPEG) {
				const enum AVPixelFormat mjpeg[] = {
				    AV_PIX_FMT_YUVJ420P, AV_PIX_FMT_YUVJ422P, AV_PIX_FMT_YUV420P, AV_PIX_FMT_YUV422P, AV_PIX_FMT_NONE
			    };
				p = mjpeg;
            } else if (ost->enc_ctx->codec_id == AV_CODEC_ID_LJPEG) {
				const enum AVPixelFormat ljpeg[] = {
				    AV_PIX_FMT_YUVJ420P, AV_PIX_FMT_YUVJ422P, AV_PIX_FMT_YUVJ444P, AV_PIX_FMT_YUV420P,
					AV_PIX_FMT_YUV422P, AV_PIX_FMT_YUV444P, AV_PIX_FMT_BGRA, AV_PIX_FMT_NONE
			    };
				p = ljpeg;
...
FilterGraph *init_simple_filtergraph(InputStream *ist, OutputStream *ost)
{
	...
	fg->outputs = (OutputFilter **)grow_array(fg->outputs, sizeof(*(fg->outputs)), &(fg->nb_outputs), fg->nb_outputs + 1); //GROW_ARRAY(fg->outputs, fg->nb_outputs);
	...
	fg->inputs = (InputFilter **)grow_array(fg->inputs, sizeof(*(fg->inputs)), &(fg->nb_inputs), fg->nb_inputs + 1); //GROW_ARRAY(fg->inputs, fg->nb_inputs);
    if (!(fg->inputs[0] = (InputFilter *)av_mallocz(sizeof(*fg->inputs[0]))))
        exit_program(1);
    fg->inputs[0]->ist   = ist;
    fg->inputs[0]->graph = fg;

	ist->filters = (InputFilter **)grow_array(ist->filters, sizeof(*(ist->filters)), &(ist->nb_filters), ist->nb_filters + 1); //GROW_ARRAY(ist->filters, ist->nb_filters);
    ist->filters[ist->nb_filters - 1] = fg->inputs[0];

	filtergraphs = (FilterGraph **)grow_array(filtergraphs, sizeof(*(filtergraphs)), &(nb_filtergraphs), nb_filtergraphs + 1); //GROW_ARRAY(filtergraphs, nb_filtergraphs);
