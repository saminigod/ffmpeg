        ...
        *(double *)dst = parse_number_or_die(opt, arg, OPT_DOUBLE, -INFINITY, INFINITY);
        ...
        if (ret < 0) {
			av_log(NULL, AV_LOG_ERROR,
				...
				arg, opt,
				av_make_error_string(error_string, AV_ERROR_MAX_STRING_SIZE, ret)//av_err2str(ret)
				   );
            return ret;
        }
    }
                         ...
                         const char *arg)
{
	...
	l->groups = (OptionGroup *)grow_array(l->groups, sizeof(*(l->groups)), &(l->nb_groups), l->nb_groups + 1);//GROW_ARRAY(l->groups, l->nb_groups);
                    ...
                    const char *key, const char *val)
{
	...
	g->opts =(Option *) grow_array(g->opts, sizeof(*g->opts), &(g->nb_opts), g->nb_opts + 1);//GROW_ARRAY(g->opts, g->nb_opts);
...
static void expand_filename_template(AVBPrint *bp, const char *exp_file_template,
                                     struct tm *tm)
{
    int c;

    while ((c = *(exp_file_template++))) {
        if (c == '%') {
			if (!(c = *(exp_file_template++)))
                break;
            switch (c) {
            case 'p':
