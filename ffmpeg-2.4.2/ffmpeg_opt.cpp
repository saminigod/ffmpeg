...
static int opt_map(void *optctx, const char *opt, const char *arg)
{
    ...
    if (map[0] == '[') {
        ...
        const char *c = map + 1;
		o->stream_maps =(StreamMap *) grow_array(o->stream_maps, sizeof(*(o->stream_maps)), &o->nb_stream_maps, o->nb_stream_maps + 1);//GROW_ARRAY(o->stream_maps, o->nb_stream_maps);
...
{
    OptionsContext *o = (OptionsContext *)optctx;
	o->attachments = (const char **)grow_array(o->attachments, sizeof(*(o->attachments)), &o->nb_attachments, o->nb_attachments + 1); //GROW_ARRAY(o->attachments, o->nb_attachments);
...
static int opt_map_channel(void *optctx, const char *opt, const char *arg)
{
	...
	o->audio_channel_maps = (AudioChannelMap *)grow_array(o->audio_channel_maps, sizeof(*(o->audio_channel_maps)), &o->nb_audio_channel_maps, o->nb_audio_channel_maps + 1); //GROW_ARRAY(o->audio_channel_maps, o->nb_audio_channel_maps);
...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic)
{
    ...
    for (i = 0; i < ic->nb_streams; i++) {
		...
		input_streams = (InputStream **)grow_array(input_streams, sizeof(*input_streams), &nb_input_streams, nb_input_streams + 1); //GROW_ARRAY(input_streams, nb_input_streams);
        ...
        case AVMEDIA_TYPE_VIDEO:
...
#if 1
			{
				...
				for (i = 0; i < o->nb_hwaccels; i++) {
					...
					if ((ret = check_stream_specifier(ic, st, spec)) > 0)
						hwaccel = (char *)o->hwaccels[i].u.str;
					else if (ret < 0)
						exit_program(1);
				}
			}
			...
			{
				int i, ret;
				for (i = 0; i < o->nb_hwaccel_devices; i++) {

					char *spec = (char *)o->hwaccel_devices[i].specifier;
					if ((ret = check_stream_specifier(ic, st, spec)) > 0)
						hwaccel = (char *)o->hwaccel_devices[i].u.str;
					else if (ret < 0)
						exit_program(1);
				}
			}
...
static int open_input_file(OptionsContext *o, const char *filename)
{
	...
	input_files = (InputFile **)grow_array(input_files, sizeof(*input_files), &nb_input_files, nb_input_files + 1); //GROW_ARRAY(input_files, nb_input_files);
    ...
    while ((e = av_dict_get(unused_opts, "", e, AV_DICT_IGNORE_SUFFIX))) {
        const AVClass *av_class = avcodec_get_class();
        const AVOption *option = av_opt_find(&av_class, e->key, NULL, 0,
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
	...
	output_streams = (OutputStream **)grow_array(output_streams, sizeof(*output_streams), &nb_output_streams, nb_output_streams + 1); //GROW_ARRAY(output_streams, nb_output_streams);
...
#if 1
	{
		...
		for (i = 0; i < o->nb_bitstream_filters; i++) {
			...
			if ((ret = check_stream_specifier(oc, st, spec)) > 0)
				bsf = (char *)o->bitstream_filters[i].u.str;
			else if (ret < 0)
				exit_program(1);
		}
	}
...
static int read_ffserver_streams(OptionsContext *o, AVFormatContext *s, const char *filename)
{
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        st->cur_dts = 0;
        st->info = (struct AVStream::info *)av_malloc(sizeof(*st->info));
...
static int open_output_file(OptionsContext *o, const char *filename)
{
	...
	output_files = (OutputFile **)grow_array(output_files, sizeof(*output_files), &nb_output_files, nb_output_files + 1); // GROW_ARRAY(output_files, nb_output_files);
...
static int opt_progress(void *optctx, const char *opt, const char *arg)
{
    ...
    if (ret < 0) {
		char av_error_string[AV_ERROR_MAX_STRING_SIZE] = { 0 };
        av_log(NULL, AV_LOG_ERROR, "Failed to open progress URL \"%s\": %s\n",
               arg, 
			   av_make_error_string(av_error_string, AV_ERROR_MAX_STRING_SIZE, ret)//av_err2str(ret)
			   );
        return ret;
    }
    progress_avio = avio;
...
#include "cmdutils_common_opts.h"
    { "f",              HAS_ARG | OPT_STRING | OPT_OFFSET |
	                    OPT_INPUT | OPT_OUTPUT,                      { /*.off       =*/ (void *) OFFSET(format) },//(void *)
        "force format", "fmt" },
    ...
    { "c",              HAS_ARG | OPT_STRING | OPT_SPEC |
                        OPT_INPUT | OPT_OUTPUT,                      { /*.off       =*/ (void *) OFFSET(codec_names) },
        "codec name", "codec" },
    { "codec",          HAS_ARG | OPT_STRING | OPT_SPEC |
                        OPT_INPUT | OPT_OUTPUT,                      { /*.off       =*/ (void *) OFFSET(codec_names) },
        "codec name", "codec" },
    { "pre",            HAS_ARG | OPT_STRING | OPT_SPEC |
                        OPT_OUTPUT,                                  { /*.off       =*/ (void *) OFFSET(presets) },
        "preset name", "preset" },
    ...
    { "map_metadata",   HAS_ARG | OPT_STRING | OPT_SPEC |
                        OPT_OUTPUT,                                  { /*.off       =*/ (void *) OFFSET(metadata_map) },
        "set metadata information of outfile from infile",
        "outfile[,metadata]:infile[,metadata]" },
    { "map_chapters",   HAS_ARG | OPT_INT | OPT_EXPERT | OPT_OFFSET |
                        OPT_OUTPUT,                                  { /*.off       =*/ (void *) OFFSET(chapters_input_file) },
        "set chapters mapping", "input_file_index" },
    { "t",              HAS_ARG | OPT_TIME | OPT_OFFSET |
                        OPT_INPUT | OPT_OUTPUT,                      { /*.off       =*/ (void *) OFFSET(recording_time) },
        "record or transcode \"duration\" seconds of audio/video",
        "duration" },
    { "to",             HAS_ARG | OPT_TIME | OPT_OFFSET | OPT_OUTPUT,  { /*.off       =*/ (void *) OFFSET(stop_time) },
        "record or transcode stop time", "time_stop" },
    { "fs",             HAS_ARG | OPT_INT64 | OPT_OFFSET | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(limit_filesize) },
        "set the limit file size in bytes", "limit_size" },
    { "ss",             HAS_ARG | OPT_TIME | OPT_OFFSET |
                        OPT_INPUT | OPT_OUTPUT,                      { /*.off       =*/ (void *) OFFSET(start_time) },
        "set the start time offset", "time_off" },
    { "accurate_seek",  OPT_BOOL | OPT_OFFSET | OPT_EXPERT |
                        OPT_INPUT,                                   { /*.off       =*/ (void *) OFFSET(accurate_seek) },
        "enable/disable accurate seeking with -ss" },
    { "itsoffset",      HAS_ARG | OPT_TIME | OPT_OFFSET |
                        OPT_EXPERT | OPT_INPUT,                      { /*.off       =*/ (void *) OFFSET(input_ts_offset) },
        "set the input ts offset", "time_off" },
    { "itsscale",       HAS_ARG | OPT_DOUBLE | OPT_SPEC |
                        OPT_EXPERT | OPT_INPUT,                      { /*.off       =*/ (void *) OFFSET(ts_scale) },
        "set the input ts scale", "scale" },
    { "timestamp",      HAS_ARG | OPT_PERFILE,                       { /*.func_arg =*/  opt_recording_timestamp },
        "set the recording timestamp ('now' to set the current time)", "time" },
    { "metadata",       HAS_ARG | OPT_STRING | OPT_SPEC | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(metadata) },
    ...
    { "re",             OPT_BOOL | OPT_EXPERT | OPT_OFFSET |
                        OPT_INPUT,                                   { /*.off       =*/ (void *) OFFSET(rate_emu) },
    ...
    { "shortest",       OPT_BOOL | OPT_EXPERT | OPT_OFFSET |
                        OPT_OUTPUT,                                  { /*.off       =*/ (void *) OFFSET(shortest) },
        "finish encoding within shortest input" },
    { "apad",           OPT_STRING | HAS_ARG | OPT_SPEC |
                        OPT_OUTPUT,                                  { /*.off       =*/ (void *) OFFSET(apad) },
        "audio pad", "" },
    ...
    { "copyinkf",       OPT_BOOL | OPT_EXPERT | OPT_SPEC |
                        OPT_OUTPUT,                                  { /*.off       =*/ (void *) OFFSET(copy_initial_nonkeyframes) },
        "copy initial non-keyframes" },
    { "copypriorss",    OPT_INT | HAS_ARG | OPT_EXPERT | OPT_SPEC | OPT_OUTPUT,   { /*.off       =*/ (void *) OFFSET(copy_prior_start) },
        "copy or discard frames before start time" },
    { "frames",         OPT_INT64 | HAS_ARG | OPT_SPEC | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(max_frames) },
        "set the number of frames to record", "number" },
    { "tag",            OPT_STRING | HAS_ARG | OPT_SPEC |
                        OPT_EXPERT | OPT_OUTPUT | OPT_INPUT,         { /*.off       =*/ (void *) OFFSET(codec_tags) },
        "force codec tag/fourcc", "fourcc/tag" },
    { "q",              HAS_ARG | OPT_EXPERT | OPT_DOUBLE |
                        OPT_SPEC | OPT_OUTPUT,                       { /*.off       =*/ (void *) OFFSET(qscale) },
    ...
    { "profile",        HAS_ARG | OPT_EXPERT | OPT_PERFILE | OPT_OUTPUT, { /*.func_arg =*/  opt_profile },
        "set profile", "profile" },
    { "filter",         HAS_ARG | OPT_STRING | OPT_SPEC | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(filters) },
        "set stream filtergraph", "filter_graph" },
    { "filter_script",  HAS_ARG | OPT_STRING | OPT_SPEC | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(filter_scripts) },
        "read stream filtergraph description from a file", "filename" },
    { "reinit_filter",  HAS_ARG | OPT_INT | OPT_SPEC | OPT_INPUT,    { /*.off       =*/ (void *) OFFSET(reinit_filters) },
    ...
    { "dump_attachment", HAS_ARG | OPT_STRING | OPT_SPEC |
                         OPT_EXPERT | OPT_INPUT,                     { /*.off       =*/ (void *) OFFSET(dump_attachment) },
    ...
    { "discard",        OPT_STRING | HAS_ARG | OPT_SPEC |
                        OPT_INPUT,                                   { /*.off       =*/ (void *) OFFSET(discard) },
        "discard", "" },
    ...
    { "r",            OPT_VIDEO | HAS_ARG  | OPT_STRING | OPT_SPEC |
                      OPT_INPUT | OPT_OUTPUT,                                    { /*.off       =*/ (void *) OFFSET(frame_rates) },
        "set frame rate (Hz value, fraction or abbreviation)", "rate" },
    { "s",            OPT_VIDEO | HAS_ARG | OPT_SUBTITLE | OPT_STRING | OPT_SPEC |
                      OPT_INPUT | OPT_OUTPUT,                                    { /*.off       =*/ (void *) OFFSET(frame_sizes) },
        "set frame size (WxH or abbreviation)", "size" },
    { "aspect",       OPT_VIDEO | HAS_ARG  | OPT_STRING | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(frame_aspect_ratios) },
        "set aspect ratio (4:3, 16:9 or 1.3333, 1.7777)", "aspect" },
    { "pix_fmt",      OPT_VIDEO | HAS_ARG | OPT_EXPERT  | OPT_STRING | OPT_SPEC |
                      OPT_INPUT | OPT_OUTPUT,                                    { /*.off       =*/ (void *) OFFSET(frame_pix_fmts) },
        "set pixel format", "format" },
    { "bits_per_raw_sample", OPT_VIDEO | OPT_INT | HAS_ARG,                      { &frame_bits_per_raw_sample },
        "set the number of bits per raw sample", "number" },
    { "intra",        OPT_VIDEO | OPT_BOOL | OPT_EXPERT,                         { &intra_only },
        "deprecated use -g 1" },
    { "vn",           OPT_VIDEO | OPT_BOOL  | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT,{ /*.off       =*/ (void *) OFFSET(video_disable) },
        "disable video" },
    { "rc_override",  OPT_VIDEO | HAS_ARG | OPT_EXPERT  | OPT_STRING | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(rc_overrides) },
    ...
    { "pass",         OPT_VIDEO | HAS_ARG | OPT_SPEC | OPT_INT | OPT_OUTPUT,     { /*.off       =*/ (void *) OFFSET(pass) },
        "select the pass number (1 to 3)", "n" },
    { "passlogfile",  OPT_VIDEO | HAS_ARG | OPT_STRING | OPT_EXPERT | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(passlogfiles) },
    ...
    { "intra_matrix", OPT_VIDEO | HAS_ARG | OPT_EXPERT  | OPT_STRING | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(intra_matrices) },
        "specify intra matrix coeffs", "matrix" },
    { "inter_matrix", OPT_VIDEO | HAS_ARG | OPT_EXPERT  | OPT_STRING | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(inter_matrices) },
        "specify inter matrix coeffs", "matrix" },
    { "chroma_intra_matrix", OPT_VIDEO | HAS_ARG | OPT_EXPERT  | OPT_STRING | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(chroma_intra_matrices) },
        "specify intra matrix coeffs", "matrix" },
    { "top",          OPT_VIDEO | HAS_ARG | OPT_EXPERT  | OPT_INT| OPT_SPEC |
                      OPT_INPUT | OPT_OUTPUT,                                    { /*.off       =*/ (void *) OFFSET(top_field_first) },
    ...
    { "force_fps",    OPT_VIDEO | OPT_BOOL | OPT_EXPERT  | OPT_SPEC |
                      OPT_OUTPUT,                                                { /*.off       =*/ (void *) OFFSET(force_fps) },
        "force the selected framerate, disable the best supported framerate selection" },
    { "streamid",     OPT_VIDEO | HAS_ARG | OPT_EXPERT | OPT_PERFILE |
                      OPT_OUTPUT,                                                { /*.func_arg =*/  opt_streamid },
        "set the value of an outfile streamid", "streamIndex:value" },
    { "force_key_frames", OPT_VIDEO | OPT_STRING | HAS_ARG | OPT_EXPERT |
                          OPT_SPEC | OPT_OUTPUT,                                 { /*.off       =*/ (void *) OFFSET(forced_key_frames) },
    ...
    { "hwaccel",          OPT_VIDEO | OPT_STRING | HAS_ARG | OPT_EXPERT |
                          OPT_SPEC | OPT_INPUT,                                  { /*.off       =*/ (void *) OFFSET(hwaccels) },
        "use HW accelerated decoding", "hwaccel name" },
    { "hwaccel_device",   OPT_VIDEO | OPT_STRING | HAS_ARG | OPT_EXPERT |
                          OPT_SPEC | OPT_INPUT,                                  { /*.off       =*/ (void *) OFFSET(hwaccel_devices) },
    ...
    { "ar",             OPT_AUDIO | HAS_ARG  | OPT_INT | OPT_SPEC |
                        OPT_INPUT | OPT_OUTPUT,                                    { /*.off       =*/ (void *) OFFSET(audio_sample_rate) },
        "set audio sampling rate (in Hz)", "rate" },
    { "ac",             OPT_AUDIO | HAS_ARG  | OPT_INT | OPT_SPEC |
                        OPT_INPUT | OPT_OUTPUT,                                    { /*.off       =*/ (void *) OFFSET(audio_channels) },
        "set number of audio channels", "channels" },
    { "an",             OPT_AUDIO | OPT_BOOL | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT,{ /*.off       =*/ (void *) OFFSET(audio_disable) },
        "disable audio" },
    ...
    { "sample_fmt",     OPT_AUDIO | HAS_ARG  | OPT_EXPERT | OPT_SPEC |
                        OPT_STRING | OPT_INPUT | OPT_OUTPUT,                       { /*.off       =*/ (void *) OFFSET(sample_fmts) },
    ...
    { "guess_layout_max", OPT_AUDIO | HAS_ARG | OPT_INT | OPT_SPEC | OPT_EXPERT | OPT_INPUT, { /*.off       =*/ (void *) OFFSET(guess_layout_max) },
      "set the maximum number of channels to try to guess the channel layout" },

    /* subtitle options */
    { "sn",     OPT_SUBTITLE | OPT_BOOL | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(subtitle_disable) },
        "disable subtitle" },
    { "scodec", OPT_SUBTITLE | HAS_ARG  | OPT_PERFILE | OPT_INPUT | OPT_OUTPUT, { /*.func_arg =*/  opt_subtitle_codec },
        "force subtitle codec ('copy' to copy stream)", "codec" },
    { "stag",   OPT_SUBTITLE | HAS_ARG  | OPT_EXPERT  | OPT_PERFILE | OPT_OUTPUT, { /*.func_arg =*/  opt_old2new }
        , "force subtitle tag/fourcc", "fourcc/tag" },
    { "fix_sub_duration", OPT_BOOL | OPT_EXPERT | OPT_SUBTITLE | OPT_SPEC | OPT_INPUT, { /*.off       =*/ (void *) OFFSET(fix_sub_duration) },
        "fix subtitles duration" },
    { "canvas_size", OPT_SUBTITLE | HAS_ARG | OPT_STRING | OPT_SPEC | OPT_INPUT, { /*.off       =*/ (void *) OFFSET(canvas_sizes) },
    ...
    /* muxer options */
    { "muxdelay",   OPT_FLOAT | HAS_ARG | OPT_EXPERT | OPT_OFFSET | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(mux_max_delay) },
        "set the maximum demux-decode delay", "seconds" },
    { "muxpreload", OPT_FLOAT | HAS_ARG | OPT_EXPERT | OPT_OFFSET | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(mux_preload) },
        "set the initial demux-decode delay", "seconds" },
    { "override_ffserver", OPT_BOOL | OPT_EXPERT | OPT_OUTPUT, { &override_ffserver },
        "override the options from ffserver", "" },

    { "bsf", HAS_ARG | OPT_STRING | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(bitstream_filters) },
    ...
    { "dn", OPT_BOOL | OPT_VIDEO | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT, { /*.off       =*/ (void *) OFFSET(data_disable) },
        "disable data" },
